...
def main():
  ...
  output_lib = os.path.join(outdir, 'libcmtd.lib')
  shutil.copyfile(os.path.join(vs_install_dir, 'libcmtd.lib'), output_lib)
  shutil.copyfile(os.path.join(vs_install_dir, 'libcmtd.pdb'),
                  os.path.join(outdir, 'libcmtd.pdb'))
  cvspath   = 'f:\\binaries\\Intermediate\\vctools\\crt_bld\\' + bindir + '\\crt\\prebuild\\build\\' + objdir + '\\mt_obj\\nativec\\\\';
  cppvspath = 'f:\\binaries\\Intermediate\\vctools\\crt_bld\\' + bindir + '\\crt\\prebuild\\build\\' + objdir + '\\xmt_obj\\cpp_obj\\\\';

  cobjfiles = [] #['malloc', 'free', 'realloc', 'heapinit', 'calloc', 'recalloc','calloc_impl']
  cppobjfiles = ['dbgheap', ]
