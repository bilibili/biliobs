...
{
    struct dstr base_dir_path;
    dstr_init(&base_dir_path);
    biliobs_get_root_path(&base_dir_path);
	for (int i = 0; i < module_patterns_size; i++){
        struct dstr bin_path;
        struct dstr data_path;
        dstr_init_copy_dstr(&bin_path, &base_dir_path);
        dstr_cat_ch(&bin_path, '\\');
        dstr_cat(&bin_path, module_bin[i]);
        dstr_cat_ch(&bin_path, 0);

        dstr_init_copy_dstr(&data_path, &base_dir_path);
        dstr_cat_ch(&data_path, '\\');
        dstr_cat(&data_path, module_data[i]);
        dstr_cat_ch(&data_path, 0);
