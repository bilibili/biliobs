...
/*
#define OUTPUT_CREATE_DESTROY_MESSAGE
*/
...
#ifdef OUTPUT_CREATE_DESTROY_MESSAGE
	printf("%s %p\n", "encoder memory allocated:", encoder);
#endif
...
static void remove_connection(struct obs_encoder *encoder)
{
	...
	//wait if some program flow is still in info.encode() function
	while (os_atomic_compare_swap_long(&encoder->context.data_usage_state, OCD_DATA_IDLE, OCD_DATA_DESTROY) == false)
		os_sleep_ms(50);
...
bool obs_encoder_initialize(obs_encoder_t *encoder)
{
	...
	encoder->context.data_usage_state = OCD_DATA_IDLE;
...
#ifdef OUTPUT_CREATE_DESTROY_MESSAGE
	printf("%s %p [%p]\n", "encoder created:", encoder, encoder->context.data);
#endif
...
void obs_encoder_shutdown(obs_encoder_t *encoder)
{
	if (encoder->context.data) {
#ifdef OUTPUT_CREATE_DESTROY_MESSAGE
		printf("%s %p [%p]\n", "encoder destroying", encoder, encoder->context.data);
#endif
		encoder->info.destroy(encoder->context.data);
		encoder->context.data = NULL;
#ifdef OUTPUT_CREATE_DESTROY_MESSAGE
		printf("%s %p [%p]\n", "encoder destroyed", encoder, encoder->context.data);
#endif
	}
}
		...
		struct encoder_frame *frame)
{
	...
	if (os_atomic_compare_swap_long(&encoder->context.data_usage_state, OCD_DATA_IDLE, OCD_DATA_INUSE))
	{
		success = encoder->info.encode(encoder->context.data, frame, &pkt,
			&received);
		os_atomic_compare_swap_long(&encoder->context.data_usage_state, OCD_DATA_INUSE, OCD_DATA_IDLE);
	}
	else
		success = false;
