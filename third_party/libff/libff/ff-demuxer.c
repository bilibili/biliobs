...
static bool find_and_initialize_stream_decoders(struct ff_demuxer *demuxer)
{
	...
	//开始：给decoder里面填充有没有音频流视频流的信息
	if (demuxer->audio_decoder != NULL)
	{
		demuxer->audio_decoder->has_audio_stream = true;
		demuxer->audio_decoder->has_video_stream = (demuxer->video_decoder != NULL);
	}
	if (demuxer->video_decoder != NULL)
	{
		demuxer->video_decoder->has_audio_stream = (demuxer->audio_decoder != NULL);
		demuxer->video_decoder->has_video_stream = true;
	}
	//结束：给decoder里面填充有没有音频流视频流的信息
...
static int64_t tmp_av_rescale_rnd(int64_t a, int64_t b, int64_t c, enum AVRounding rnd) {
	int64_t r = 0;
	assert(c > 0);
	assert(b >= 0);
	...
	if (a < 0 && a != INT64_MIN)
		return -(uint64_t)tmp_av_rescale_rnd(-FFMAX(a, -INT64_MAX), b, c, rnd ^ ((rnd >> 1) & 1));
...
static int64_t tmp_av_rescale_q(int64_t a, AVRational bq, AVRational cq){
	int64_t b = bq.num * (int64_t)cq.den;
	int64_t c = cq.num * (int64_t)bq.den;
	return tmp_av_rescale_rnd(a, b, c, AV_ROUND_NEAR_INF);
}
static bool handle_seek(struct ff_demuxer *demuxer)
{
	...
	if (demuxer->seek_request) {
		...
		ret = av_seek_frame(demuxer->format_context,
				...
				demuxer->seek_flags);
		if (ret < 0){
			seek_target = tmp_av_rescale_q(demuxer->seek_pos,
			AV_TIME_BASE_Q,
			seek_stream->time_base);
			ret = av_seek_frame(demuxer->format_context,
				0, seek_target,
...
static void *demux_thread(void *opaque)
{
	...
	double timestampUnit = 1.0;

	struct ff_packet packet = {0};

	bool is_first_frame = true; //标志位，当前解码的是不是第一帧
	bool is_seeked = false; //标志位，当前是不是刚刚进行过seek操作
	int frameCount = 0;
	...
	//时间单位
	if (demuxer->video_decoder && demuxer->video_decoder->stream)
		timestampUnit = av_q2d(demuxer->video_decoder->stream->time_base);

	while (!demuxer->abort) {
		is_seeked = demuxer->seek_request;
		...
		if (packet.base.pts == AV_NOPTS_VALUE)
			packet.base.pts = 0;
		packet.ts_unit = timestampUnit;
		packet.is_first_frame = is_first_frame;
		packet.is_seeked = is_seeked;
