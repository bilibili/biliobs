...
{
    bool exist;
	gc->global_hook_info_map = get_hook_info(gc->process_id, &exist);
...
static obs_properties_t *game_capture_properties(void *data)
{
	...
	/*
	使用辅助进程过滤掉确认不含有d3d或者opengl模块的进程
	在64位系统下，32位的直播机无法获得64位进程中的模块列表等信息
	所以要用专门的64位程序来获取
	获取到的结果通过匿名管道传回来
	*/
	...
	DWORD* pids;
	int pidCount = 0;
	pids = bmalloc(sizeof(DWORD) * pidCountLimit);

	const char* u8gamelistfilter_exe;
	if (is_64bit_windows())
		u8gamelistfilter_exe = obs_module_file("game-list-filter-64.exe");
	else
		u8gamelistfilter_exe = obs_module_file("game-list-filter-32.exe");

	wchar_t* wcsgamelistfiltercmd = bmalloc(65536 * sizeof(wchar_t));

	os_utf8_to_wcs(u8gamelistfilter_exe, 0, wcsgamelistfiltercmd, 65536);
	if (wcsgamelistfiltercmd != 0)
	{
		wchar_t pipeHandle[64];
		HANDLE hPipeRead, hPipeWrite;
		if (CreatePipe(&hPipeRead, &hPipeWrite, NULL, 0))
		{
			SetHandleInformation(hPipeWrite, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT);

			swprintf(pipeHandle, sizeof(pipeHandle) / sizeof(*pipeHandle), L"%d", hPipeWrite);
			wcscat(wcsgamelistfiltercmd, L" ");
			wcscat(wcsgamelistfiltercmd, pipeHandle);

			STARTUPINFOW si = { 0 };
			si.cb = sizeof(si);
			PROCESS_INFORMATION pi = { 0 };
			if (CreateProcessW(0, wcsgamelistfiltercmd, 0, 0, TRUE, CREATE_NO_WINDOW, 0, 0, &si, &pi))
			{
				...
				*/
				...
				{
					DWORD readBytes;
					if (ReadFile(hPipeRead, &pids[pidCount], sizeof(DWORD), &readBytes, NULL) == 0)
						break;
					if (readBytes == 0)
						break;
		...
		bfree(wcsgamelistfiltercmd);
	}

	fill_game_window_list(p, INCLUDE_MINIMIZED, pids, pidCount);
	bfree(pids);
