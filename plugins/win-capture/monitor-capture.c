		...
		obs_data_t *settings)
{
	...
	RECT rectToCapture = monitor.rect;

	if (capture->is_limit_rect)
	{
		if (capture->is_limit_rect)
		{
			//计算rectToCapture和limit_rect的相交部分
			...
			int bottom = min(capture->limit_rect.bottom, rectToCapture.bottom);
	...
	width  = rectToCapture.right  - rectToCapture.left;
	height = rectToCapture.bottom - rectToCapture.top;

	dc_capture_init(&capture->data, rectToCapture.left, rectToCapture.top,
		...
		obs_data_t *settings)
{
	...
	capture->is_limit_rect = obs_data_get_bool(settings, "is_limit_rect");
	capture->limit_rect.left = obs_data_get_int(settings, "limit_x");
	capture->limit_rect.right = capture->limit_rect.left + obs_data_get_int(settings, "limit_cx");
	capture->limit_rect.top = obs_data_get_int(settings, "limit_y");
	capture->limit_rect.bottom = capture->limit_rect.top + obs_data_get_int(settings, "limit_cy");
