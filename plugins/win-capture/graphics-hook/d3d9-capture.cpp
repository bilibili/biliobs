...
static inline bool shex_init_d3d11()
{
	...
	hr = create_device(adapter, D3D_DRIVER_TYPE_HARDWARE, nullptr,
			0, feature_levels,
...
static int (__stdcall *d3d9device_release)(IDirect3DDevice9*) = 0;

static int __stdcall hooked_d3d9device_release(IDirect3DDevice9* d3d9dev)
{
	int r = 1;
	if (d3d9device_release) {
		r = d3d9device_release(d3d9dev);
	}
...
static void d3d9_init(IDirect3DDevice9 *device)
{
	...
	if (GetModuleHandleW(L"300.exe") != 0)
	{
		void** d3d9dev_vtbl = *((void***)device);
		if (d3d9dev_vtbl[2] != hooked_d3d9device_release)
			*(void**)&d3d9device_release = d3d9dev_vtbl[2];

		d3d9dev_vtbl[2] = hooked_d3d9device_release;
	}
