...
struct scroll_filter_data {
	...
	//如果滚动速度是0，那么要不要防止它循环多次显示以填满整个范围
	//比如：文字
	//显示为
	//文字文字文字文字文字文
...
static void scroll_filter_update(void *data, obs_data_t *settings)
{
	...
	filter->disable_repeat_if_no_speed = obs_data_get_bool(settings, "disable_repeat_if_no_speed");
