...
void HDevice::Receive(bool isVideo, IMediaSample *sample)
{
	...
	if (!hasTime)
	{
		...
		{
			if (predictFrameDuration == 0)
				predictFrameDuration = currentTime - lastFrameStartTime;
			else
				predictFrameDuration = predictFrameDuration * 0.6 + (currentTime - lastFrameStartTime) * 0.4;

			startTime = lastFrameStartTime + predictFrameDuration;
			stopTime = startTime;
		}
...
Result HDevice::Start()
{
	...
	if (FAILED(hr)) {
		wchar_t tip[256] = { 0 };
		swprintf_s(tip, L"DShow Err:(0x%08lX): %s", hr, ConvertHRToChinese(hr).c_str());
		MessageBox(nullptr, tip, L"DShow Camera", MB_OK);
