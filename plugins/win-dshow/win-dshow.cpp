...
struct DShowInput {
	...
	WinHandle thread;
	CriticalSection mutex;
	vector<std::pair<Action, uint64_t> > actions;

	inline void QueueAction(Action action)
	{
		CriticalScope scope(mutex);
		actions.push_back( make_pair(action, _time64(0)) );
	...
	{
		{
			CriticalScope scope(mutex);
			actions.resize(1);
			actions[0].first = Action::Shutdown;
			actions[0].second = _time64(0);
		}
...
void DShowInput::DShowLoop()
{
	...
	while (true) {
		...
		uint64_t actionTime = 0;
		{
			CriticalScope scope(mutex);
			if (actions.size()) {
				action = actions.front().first;
				actionTime = actions.front().second;
				actions.erase(actions.begin());
			}
		}

		if ((lastAction == Action::ConfigAudio || lastAction == Action::ConfigVideo)
			&& lastAction == action
			&& actionTime < _time64(0)
			)
		{
			continue;
		}
